...
class NDCMultinomialRaysampler(MultinomialRaysampler):
    """
    Replicate pytorch3d 0.4.0 legacy NDCGridRaysampler
    """
...
class SemanticNeRFRaysampler(torch.nn.Module):
    """
    Implements the raysampler of NeRF.
    ...
    def __init__(
        ...
        # Initialize the Monte Carlo ray sampler.
        self._mc_raysampler = WeightedMonteCarloRaysampler(
            min_x=-1.0,
            max_x=1.0,
            min_y=-1.0,
            max_y=1.0,
    ...
    def forward(
        ...
        **kwargs,
        ...
        #fg_pixels = ((target_sem_label == 1) | (target_sem_label == 2)).sum()
        #all_pixels = 2 * 384 * 512
        #print((fg_pixels / all_pixels).item())
